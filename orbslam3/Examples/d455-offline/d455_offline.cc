...
int main(int argc, char **argv) {
    ...
    */
    if (argc < 3 || argc > 4) {
        cerr << endl
             << "Usage: ./d455_offline path_to_vocabulary path_to_settings path_to_sequence"
             << endl;
        return 1;
    }
    ...
    cout<<"sequence file: "<<file_name<<endl;

    // ===INIT YOLO MODEL=======================================================
    //yolo//cout<<"Loading YOLO model ..."<<endl;
    //yolo//string onnx_model_path = "yolo/models/best-seg-480-640.onnx";
    //yolo//cv::Size modelInputShape(640, 480);
    //yolo//string classesTxtFile = "yolo/classes.txt";
    //yolo//bool runWithCuda = true;

    //yolo//YOLO::Inference inf(onnx_model_path, modelInputShape, classesTxtFile, runWithCuda);
    // =========================================================================
    ...
    */
    ...
    while (!SLAM.isShutDown())
    {
        ...
        //yolo//cv::imshow("original frame", im);
        //yolo//cout<<"frame size in original frame"<<endl<<"width: "<<im.cols<<" height: "<<im.rows<<endl;
        // ...end DISPLAY ORIGINAL FRAME

        // EXECUTE YOLO ON THE ACQUIRED FRAME
        //yolo//cv::Mat yolo_frame;
        //yolo//cv::cvtColor(im, yolo_frame, cv::COLOR_BGR2RGB);
        //yolo//cv::resize(yolo_frame, yolo_frame, cv::Size(640, 480));

        //yolo//std::vector<YOLO::Detection> output = inf.runInference(yolo_frame);

        //yolo//int detections = output.size();
        //yolo//cout<<"Number of detections: "<<detections<<endl;

        //yolo//for (int i=0; i<detections; ++i){
            //yolo//YOLO::Detection detection = output[i];

            //yolo//cv::Rect box = detection.box;
            //yolo//cv::Scalar color = detection.color;
            //yolo//cv::Mat mask = detection.boxMask;

            // Detection box
            //yolo//cv::rectangle(yolo_frame, box, color, 2);

            // Detection mask
            //yolo//yolo_frame(box).setTo(color, mask);

            // Detection box text
            //yolo//std::string classString = detection.className + ' ' + to_string(detection.confidence).substr(0, 4);
            //yolo//cv::Size textSize = cv::getTextSize(classString, cv::FONT_HERSHEY_DUPLEX, 1, 2, 0);
            //yolo//cv::Rect textBox(box.x, box.y - 40, textSize.width + 10, textSize.height + 20);

            //yolo//cv::rectangle(yolo_frame, textBox, color, cv::FILLED);
            //yolo//cv::putText(yolo_frame, classString, cv::Point(box.x + 5, box.y - 10), cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(0, 0, 0), 2, 0);
        //yolo//}

        // This is only for preview purposes
        //yolo//float scale = 1;
        //cv::resize(yolo_frame, yolo_frame, cv::Size(yolo_frame.cols*scale, yolo_frame.rows*scale));
        //yolo//cv::imshow("yolo inference", yolo_frame);
    ...
    cout<<"Saving trajectory TUM"<<endl;
    SLAM.SaveTrajectoryTUM("CameraTrajectoryOrbslam3.txt", log_camera_file_path);
    ...
    // See format details at: http://vision.in.tum.de/data/datasets/rgbd-dataset
    */
    SLAM.SaveKeyFrameTrajectoryTUM("KeyFrameTrajectoryOrbslam3.txt");
    // ... added by me ...
}
