...
void System::SaveTrajectoryTUM(const string &filename, const string &log_camera_file_path)
{
    /*
    Parameters:
        # filename: name of the txt file where the camera trajectory is saved
        # log_camera_file_path: name of the csv file where the camera trajectory+timestamp is saved
    */
    ...
    // open csv file
    ofstream log_camera_file;
    log_camera_file.open(log_camera_file_path);
    log_camera_file<<"timestamp,x,y,z"<<endl;
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        f << setprecision(6) << *lT << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        log_camera_file<<chrono::duration_cast<chrono::microseconds>(chrono::steady_clock::now().time_since_epoch()).count()<<","<<twc(0)<<","<<twc(1)<<","<<twc(2)<<endl;
    }
    f.close();
    log_camera_file.close();
    // cout << endl << "trajectory saved!" << endl;
}
